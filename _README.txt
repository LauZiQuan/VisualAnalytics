...
1) Import data in the IS428HomevizHomies.sql file provided into MySQL Workbench. 
2) Open up the DashboardBE Folder on command prompt and run `npm start`. Server will be listening on port 3000. 
3) Open the Dashboard on VSCode and run Live Server (by clicking Go Live on the bottom bar). The project will start on port 5500.